...
"""program"""
__author__ = "MINDsLAB"
__date__ = "creation: 2017-11-30, modification: 2018-01-11"
...
from cfg.config import STT_CONFIG, MYSQL_DB_CONFIG, MSSQL_DB_CONFIG, MASKING_CONFIG
...
class MySQL(object):
    ...
    def select_rcdg_id(self, **kwargs):
        query = """
            SELECT
                RCDG_ID
            FROM
...
def error_process(logger, mysql, poli_no, ctrdt, stt_req_dtm, status):
    """
    Error process
    :param          logger:                 Logger
    :param          mysql:                  MySQL
    ...
    output_dir_path = "{0}/{1}/{2}/{3}/{4}_{5}/TB_QA_STT_TM_CNTR_INFO".format(
        STT_CONFIG['stt_output_path'], ctrdt[:4], ctrdt[4:6], ctrdt[6:8], poli_no, ctrdt)
    ...
    output_dict['stt_prgst_cd'] = status
...
def wav_file_not_found_process(logger, mysql, stt_prgst_cd, poli_no, ctrdt, stt_req_dtm):
    ...
    temp_output_dir_name = "{0}/{1}/{2}/{3}/{4}".format(
        STT_CONFIG['stt_output_path'], ctrdt[:4], ctrdt[4:6], ctrdt[6:8], output_dir_name)
    output_dir_path = "{0}/TB_QA_STT_TM_CNTR_INFO".format(temp_output_dir_name)
    if os.path.exists(temp_output_dir_name):
        del_garbage(logger, temp_output_dir_name)
    os.makedirs(output_dir_path)
...
def make_file_for_tb_qa_stt_tm_cntr_info(logger, poli_no, ctrdt, stt_req_dtm):
    """
    Make file for TB_SQ_STT_TM_CNTR_INFO
    :param          logger:                     Logger
    :param          poli_no:                    POLI_NO(증서 번호)
    :param          ctrdt:                      CTRDT(계약 일자)
    :param          stt_req_dtm:                STT_REQ_DTM(CS 요청 일시)
    """
    logger.info("15. Make db upload file for TB_QA_STT_TM_CNTR_INFO")
    output_dir_path = "{0}/TB_QA_STT_TM_CNTR_INFO".format(STT_OUTPUT_DIR_PATH)
    if not os.path.exists(output_dir_path):
        os.makedirs(output_dir_path)
    tm_cntr_info_output_file_path = "{0}/{1}_{2}_tb_qa_stt_tm_cntr_info.txt".format(output_dir_path, poli_no, ctrdt)
    tm_cntr_info_output_file = open(tm_cntr_info_output_file_path, 'w')
    output_dict = collections.OrderedDict()
    output_dict['poli_no'] = poli_no
    output_dict['ctrdt'] = ctrdt
    output_dict['stt_prgst_cd'] = '05'
    output_dict['stt_req_dtm'] = stt_req_dtm
    output_dict['ta_cmdtm'] = str(datetime.now())
    output_list = list(output_dict.values())
    print >> tm_cntr_info_output_file, "\t".join(output_list)
    tm_cntr_info_output_file.close()
    db_upload_dir_path = "{0}/{1}.tmp/TB_QA_STT_TM_CNTR_INFO".format(STT_CONFIG['db_upload_path'], OUTPUT_DIR_NAME)
    if not os.path.exists(db_upload_dir_path):
        os.makedirs(db_upload_dir_path)
    shutil.copy(tm_cntr_info_output_file_path, db_upload_dir_path)
    ...
    for dir_path, sub_dirs, files in w_ob:
        for file_name in files:
            ...
            for line in detail_file_list:
                ...
                msk_info_lit = str(masking_output_dict[line_num]) if msk_dtc_yn == "Y" else "None"
                output_dict = collections.OrderedDict()
                output_dict['poli_no'] = poli_no
                output_dict['ctrdt'] = ctrdt
                ...
                output_dict['msk_info_lit'] = msk_info_lit
                output_list = list(output_dict.values())
                line_num += 1
...
def move_output(logger, ctrdt):
    ...
    del_garbage(logger, pcm_dir_path)
    wav_dir_path = "{0}/wav".format(STT_TEMP_DIR_PATH)
    del_garbage(logger, wav_dir_path)
    ...
    for rcdf_file_nm in rec_file_dict.keys():
        ...
        if os.path.exists(target_mono_file_path):
            if len(chn_tp) < 1:
                chn_tp = 'M'
            if chn_tp == 'S':
                err_str = "Both mono and stereo existed"
                raise Exception(err_str)
            ...
            logger.info("CHN_TP = M, {0} -> {1}".format(target_mono_file_path, STT_TEMP_DIR_PATH))
            ...
            logger.info("CHN_TP = S, {0} -> {1}".format(target_rx_file_path, STT_TEMP_DIR_PATH))
            logger.info("CHN_TP = S, {0} -> {1}".format(target_tx_file_path, STT_TEMP_DIR_PATH))
            ...
            logger.info("CHN_TP = M, {0} -> {1}".format(incident_target_mono_file_path, STT_TEMP_DIR_PATH))
            ...
            logger.info("CHN_TP = S, {0} -> {1}".format(incident_target_rx_file_path, STT_TEMP_DIR_PATH))
            logger.info("CHN_TP = S, {0} -> {1}".format(incident_target_tx_file_path, STT_TEMP_DIR_PATH))
        else:
...
def update_status_and_select_rec_file(logger, mysql, mssql, poli_no, ctrdt, stt_req_dtm):
    ...
    rec_file_dict = dict()
    rcdg_id_list = mysql.select_rcdg_id(
        poli_no=poli_no,
        ctrdt=ctrdt
    )
...
def processing(job):
    ...
    try:
        ...
        exc_info = traceback.format_exc()
        logger.error(exc_info)
        error_process(logger, mysql, poli_no, ctrdt, stt_req_dtm, '04')
        delete_garbage_file(logger)
        mssql.disconnect()
        mysql.disconnect()
        for handler in logger.handlers:
        ...
        sys.exit(1)
    try:
        # 15. Make DB upload file
        make_file_for_tb_qa_stt_tm_cntr_info(logger, poli_no, ctrdt, stt_req_dtm)
        # 16. Update status
        logger.info("16. Update status to STTA END(05)")
        mysql.update_stt_prgst_cd(
            stt_prgst_cd='05',
            stt_req_dtm=stt_req_dtm,
            poli_no=poli_no,
        ...
        logger.info("17. Update TA_PRC_YN [TB_QA_STT_TM_CNTR_SCRT_INFO, TB_QA_STT_TM_CNTR_SCRT_PROD_INFO]")
        mysql.update_ta_prc_yn_from_scrt_info(
            poli_no=poli_no,
            ctrdt=ctrdt
        )
    ...
    mssql.disconnect()
    mysql.disconnect()
    # 17. Rename temp DB upload directory
    logger.info("18. Rename temp DB upload directory")
    temp_db_upload_dir_path = "{0}/{1}.tmp".format(STT_CONFIG['db_upload_path'], OUTPUT_DIR_NAME)
    db_upload_dir_path = "{0}/{1}".format(STT_CONFIG['db_upload_path'], OUTPUT_DIR_NAME)
    logger.info("{0} -> {1}".format(temp_db_upload_dir_path, db_upload_dir_path))
    if os.path.exists(db_upload_dir_path):
        del_garbage(logger, db_upload_dir_path)
    os.rename(temp_db_upload_dir_path, db_upload_dir_path)
