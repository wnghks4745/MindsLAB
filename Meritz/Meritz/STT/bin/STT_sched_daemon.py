...
LOG_FILE = '/data1/MindsVOC/TA/Meritz/logs/STT_shced_daemon.log'
...
def update_bad_status_cd():
    ...
    cmd = '/data1/MindsVOC/CS/Meritz/bin/StatCdMng.py'
    sp = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    response_out, response_err = sp.communicate()
...
class SchedDaemon(Daemon):
    def run(self):
        ...
        while True:
            ...
            logger.info('Next STT_schedule time is %s' % str(future_dt))
...
if __name__ == '__main__':
    ...
    daemon = SchedDaemon('/tmp/STT_sched_daemon.pid', stdout='/tmp/sd.log', stderr='/tmp/sd.log')
    # daemon.run()
    # sys.exit(0)
