...
def execute_hmd(sentence, nlp_sentence, hmd_dict):
    ...
    tmp_nlp_sent = ' {0} '.format(nlp_sentence)
    ...
    with open(target_file_path) as target_file:
        for line in target_file:
            ...
            try:
                ...
                continue
            output_list.append((sentence, detect_category_dict))
    output_dict = dict()
    for sentence, detect_category_dict in output_list:
        if detect_category_dict:
            for category, value in detect_category_dict.items():
                output_cate = args.cate_delimiter.join(category.split(args.cate_delimiter)[:-3])
                idx_id = category.split(args.cate_delimiter)[-3]
                end_idx = category.split(args.cate_delimiter)[-2]
                idx = category.split(args.cate_delimiter)[-1]
                if (output_cate, idx_id) not in output_dict:
                    output_dict[(output_cate, idx_id)] = (end_idx, [idx])
                else:
                    output_dict[(output_cate, idx_id)][1].append(idx)
    category_dict = dict()
    ...
    for key, value in output_dict.items():
        ...
        for cnt in range(1, end_idx + 1):
            if str(cnt) in idx_list:
                ...
                flag = False
        if flag:
            hmd_output_check_dict[key[0] + args.cate_delimiter + key[1]] = 1
            category_dict[key[0]] = 1
    output_dir_path = '{0}/test_hmd_output'.format(os.path.dirname(target_file_path))
    if not os.path.exists(output_dir_path):
        os.makedirs(output_dir_path)
    output_file = open('{0}/{1}_hmd'.format(output_dir_path, os.path.basename(target_file_path)), 'w')
    if category_dict:
        for category in category_dict.keys():
            print >> output_file, '{0}\t{1}'.format(os.path.basename(target_file_path), category)
    else:
        print >> output_file, '{0}\tNone'.format(os.path.basename(target_file_path))
    all_output_dir_path = '{0}/test_hmd_all_output'.format(os.path.dirname(target_file_path))
    if not os.path.exists(all_output_dir_path):
        os.makedirs(all_output_dir_path)
    all_output_file = open('{0}/{1}_hmd'.format(all_output_dir_path, os.path.basename(target_file_path)), 'w')
    for sentence, detect_category_dict in output_list:
        if detect_category_dict:
            for category, value in detect_category_dict.items():
                check_cate = args.cate_delimiter.join(category.split(args.cate_delimiter)[:-2])
                if check_cate in hmd_output_check_dict:
                    for dtc_keyword, hmd_rule, output_nlp_sent in value:
                        print >> all_output_file, '{0}\t{1}\t{2}'.format(category, dtc_keyword, sentence)
                else:
                    print >> all_output_file, '{0}\t{1}\t{2}'.format('Dele', 'Dele', sentence)
        else:
            print >> all_output_file, '{0}\t{1}\t{2}'.format('None', 'None', sentence)
