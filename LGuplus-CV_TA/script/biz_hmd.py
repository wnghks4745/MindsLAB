    ...
    if args.output:
        ...
        for sentence, detect_category_dict in output_list:
            if detect_category_dict:
                for category, value in detect_category_dict.items():
                    for dtc_keyword, hmd_rule, output_nlp_sent in value:
                        print >> output_file, '{0}\t{1}\t{2}'.format(category, dtc_keyword, sentence)
            else:
                print >> output_file, '{0}\t{1}\t{2}'.format('None', 'None', sentence)
        output_file.close()
...
def main(args):
    ...
    try:
        ...
        if args.text:
            if args.nlp_text:
                detect_category_dict = execute_hmd(args.text, args.nlp_text, hmd_dict)
                if detect_category_dict:
                    for category, value in detect_category_dict.items():
                        for dtc_keyword, hmd_rule, output_nlp_sent in value:
