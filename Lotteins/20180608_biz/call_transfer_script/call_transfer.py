...
class Oracle(object):
    ...
    def select_call_transfer_call_list(self, call_date):
        query = """
            SELECT
                AA.CALL_ID,
    ...
    def insert_transfer_call_analysis_hmd_result(self, call_date, return_list):
    ...
    def insert_transfer_call_analysis_hmd_result_by_executemany(self, values_list):
        query = """
            INSERT INTO CS_TRANSFER_CALL_ANALYSIS_TB
            (
                PIPELINE_EVENT_ID,
    ...
    def delete_call_transfer_hmd_result(self, call_date):
        try:
            query = """
                DELETE FROM
                    CS_TRANSFER_CALL_ANALYSIS_TB
                WHERE 1=1
                    AND CALL_DATE = TO_DATE(:1, 'YYYY-MM-DD')
            """
            bind = (
...
def extract_call_transfer_hmd(logger, idx, pro_id, model_params, target_results, return_dict):
    """
    Extract call transfer hmd
    :param              logger:                     Logger
    :param              idx:                        Index
        ...
        for cls in hmd_result.keys():
            try:
                ...
                if len(value_list) == 10000:
                    ...
                    oracle.insert_transfer_call_analysis_hmd_result_by_executemany(value_list)
                    value_list = list()
            except Exception:
                print traceback.format_exc()
                logger.error(traceback.format_exc())
    ...
    try:
        ...
        if target_results:
            ...
            for idx in range(0, len(target_results), split_cnt):
                proc = Process(target=extract_call_transfer_hmd, args=(
