...
"""program"""
__author__ = "MINDsLAB"
__date__ = "creation: 2018-05-18, modification: 2018-05-18"
...
class Oracle(object):
    ...
    def delete_data_to_cm_tm_poli_no_tb(self, today_date, delete_date):
        global DELETE_CNT
        try:
            query = """
                DELETE
                    CM_TM_POLI_NO_TB
                WHERE 1=0
                    OR TO_DATE(CTRDT, 'YYYY/MM/DD') = TO_DATE(:1, 'YYYY/MM/DD')
                    OR TO_DATE(CTRDT, 'YYYY/MM/DD') < TO_DATE(:2, 'YYYY/MM/DD')
            """
            bind = (
                today_date,
                delete_date,
    ...
    def insert_data_to_cm_tm_poli_no_tb(self, **kwargs):
        try:
            query = """
                INSERT INTO CM_TM_POLI_NO_TB (
                        CTRDT,
                        POLI_NO,
                        IP_CD,
                        RUSER_ID,
                    ...
                    VALUES (
                        :1, :2, :3, :4, :5, :6, M2U_MASKING_NAME(:7), 
                        M2U_CRYPTO_SHA256_ENCRYPT(:8), SYSDATE, SYSDATE, :9, :9
                    )
            """
            bind = (
                kwargs.get('ctrdt'),
    ...
    def update_to_cm_call_meta_tb(self, **kwargs):
        global UPDATE_CNT
        try:
            query = """
                UPDATE
                ...
                SET
                    ...
                    UPDATOR_ID = :3
                WHERE 1=1
                    AND CALL_DATE BETWEEN TO_DATE(:4, 'YYYY/MM/DD')
                                    AND SYSDATE
                    AND RUSER_ID = :5
                    AND CU_NAME = M2U_MASKING_NAME(:6)
                    AND CU_NAME_HASH = M2U_CRYPTO_SHA256_ENCRYPT(:7)
                    AND POLI_NO IS NULL
                    AND IP_CD IS NULL
                    AND PROJECT_CODE = 'PC0002'
                    AND BUSINESS_DCD = 'TD0001'
            """
            bind = (
                kwargs.get('poli_no'),
                kwargs.get('ip_cd'),
                kwargs.get('regp_cd'),
                kwargs.get('range_start_date'),
                kwargs.get('ruser_id'),
                kwargs.get('cu_name'),
                kwargs.get('cu_name'),
            )
            ...
            if self.cursor.rowcount > 0:
                ...
                self.logger.info("UPDATE CM_CALL_META_TB COUNT : {0}".format(self.cursor.rowcount))
                self.conn.commit()
                return True
            else:
                self.conn.rollback()
...
def error_process(logger, file_path):
    ...
    try:
        ...
        error_poli_file_path = '{0}/{1}'.format(error_dir_path, os.path.basename(file_path))
        if os.path.exists(error_poli_file_path):
            del_garbage(logger, error_poli_file_path)
        logger.error('Error poli file move {0} -> {1}'.format(file_path, error_dir_path))
...
def upload_data_to_db(logger, oracle, poli_file_dict):
    """
    Upload data to database
    :param      logger:             Logger
    :param      oracle:             Oracle
    :param      poli_file_dict:     POLI file dictionary
    """
    global UPLOAD_CNT
    logger.debug("Poli data upload to DB")
    for table_name, file_path_list in poli_file_dict.items():
        for file_path in file_path_list:
            try:
                logger.debug('Open file. [{0}]'.format(file_path))
                file_date = os.path.basename(file_path)[:8]
                poli_file = open(file_path, 'r')
                try:
                ...
                if table_name == 'CM_TM_POLI_NO_TB':
                    ...
                    oracle.delete_data_to_cm_tm_poli_no_tb(today_date, delete_date)
                    for line in poli_file:
                        line_list = line.split('|')
                        ctrdt = line_list[0]
                        poli_no = line_list[5]
                        logger.debug('INSERT {0} -> CTRDT : {1}, POLI_NO : {2}'.format(table_name, ctrdt, poli_no))
                        oracle.insert_data_to_cm_tm_poli_no_tb(
                            ctrdt=ctrdt,
                            poli_no=poli_no,
                            ruser_id=line_list[1].strip(),
                            ruser_name=line_list[2].strip(),
                            cu_id=line_list[3].strip()[:7],
                            cu_name=line_list[4].strip(),
                            ip_cd=line_list[6].strip(),
                            regp_cd='POLI_BAT'
                        )
                        range_start_date = (datetime.strptime(ctrdt, '%Y%m%d') - timedelta(
                            days=int(CONFIG['mapping_date_range']))).strftime('%Y%m%d')
                        """
                        oracle.update_to_cm_call_meta_tb(
                ...
                poli_file.close()
                poli_output_dir_path = '{0}/{1}/{2}/{3}'.format(CONFIG['poli_output_path'], DT[:4], DT[4:6], DT[6:8])
                poli_output_file_path = os.path.join(poli_output_dir_path, os.path.basename(file_path))
                if not os.path.exists(poli_output_dir_path):
                    os.makedirs(poli_output_dir_path)
                if os.path.exists(poli_output_file_path):
                    del_garbage(logger, poli_output_file_path)
                logger.debug("Move poli file to output directory")
                shutil.move(file_path, poli_output_dir_path)
                oracle.conn.commit()
            except Exception:
                exc_info = traceback.format_exc()
                logger.error(exc_info)
...
def make_file_dict(logger):
    ...
    """
    global POLI_TARGET_CNT
    logger.debug('Make file list')
    target_file_dict = CONFIG['poli_file_path_list']
    ...
    for target_table, target_file_path in target_file_dict.items():
        ...
        POLI_TARGET_CNT += len(target_file_path_list)
...
def processing():
    ...
    try:
        # Make poli file dict
        poli_file_dict = make_file_dict(logger)
        # Upload poli data to db
        upload_data_to_db(logger, oracle, poli_file_dict)
        logger.info(
            "Total POLI target count = {0}, delete count = {1}, insert count = {2}, meta update count = {3}, error count = {4}, The time required = {5}".format(
                POLI_TARGET_CNT, DELETE_CNT, UPLOAD_CNT, UPDATE_CNT,ERROR_CNT, elapsed_time(DT)))
...
def main():
    """
    This is a program that update Poli data to Oracle DB
    """
    global ST
    global DT
    ts = time.time()
