...
class Oracle(object):
    ...
    def insert_perfect_sales_hmd_result(self, overlap_checked_result):
        for item_dict in overlap_checked_result:
            query = """
            ...
            bind = (
                ...
                item_dict.get('stt_result_detail_id'),
            )
            self.cursor.execute(query, bind)
...
def upload_perfect_sales(logger, call_date, target_results, return_list):
    """
    Upload perfect sale
    :param          logger:                     Logger
    ...
    for target_result in target_results:
        try:
            ...
            stt_result_detail_id = target_result[8]
...
def processing(target_call_date):
    ...
    try:
        ...
        if target_results:
            logger.info("CS perfect sales line count -> {0}".format(len(target_results)))
            proc_list = list()
            split_cnt = len(target_results) / 10
            manager = Manager()
            return_list = manager.list()
            for idx in range(0, len(target_results), split_cnt):
                proc = Process(target=upload_perfect_sales, args=(
                    logger, call_date, target_results[idx:split_cnt+idx], return_list), )
