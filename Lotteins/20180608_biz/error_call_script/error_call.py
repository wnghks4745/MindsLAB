...
class Oracle(object):
    ...
    def select_error_call_list(self, call_date):
        query = """
            SELECT
                AA.CALL_ID,
    ...
    def insert_error_call_hmd_result(self, call_date, return_list):
        overlap_check_dict = dict()
        for hmd_result, temp_dict in return_list:
            try:
                for cls in hmd_result.keys():
                    if temp_dict['call_id'] in overlap_check_dict:
                        break
                    overlap_check_dict[temp_dict['call_id']] = 1
                    query = """
    ...
    def insert_error_call_hmd_result_by_executemany(self, values_list):
        query = """
        INSERT INTO CS_ERROR_CALL_ANALYSIS_TB
        (
            PIPELINE_EVENT_ID,
    ...
    def delete_error_call_hmd_result(self, call_date):
        try:
            query = """
                DELETE FROM
                    CS_ERROR_CALL_ANALYSIS_TB
                WHERE 1=1
                    AND CALL_DATE = TO_DATE(:1, 'YYYY-MM-DD')
            """
            bind = (
...
def extract_error_call_hmd(logger, idx, pro_id, model_params, target_results, return_dict):
    """
    Extract error call hmd
    :param              logger:                     Logger
    :param              idx:                        Index
    ...
    for target_result in target_results:
        try:
            ...
            sentence = target_result[2]
            stt_result_detail_id = target_result[3]
            nlp_result_id = target_result[4]
            nlp_sentence = target_result[5]
        ...
        for cls in hmd_result.keys():
            try:
                ...
                if len(value_list) == 10000:
                    ...
                    oracle.insert_error_call_hmd_result_by_executemany(value_list)
                    value_list = list()
            except Exception:
                print traceback.format_exc()
                logger.error(traceback.format_exc())
...
def processing(target_call_date):
    ...
    try:
        ...
        logger.info("CALL_DATE = {0}, HMD = {1}".format(call_date, model_params))
        print "1. Select target data"
        logger.info("1. Select target data")
        target_results = oracle.select_error_call_list(call_date)
        oracle.disconnect()
        if target_results:
            print "CS target data count = {0}".format(len(target_results))
            logger.info("CS target data count = {0}".format(len(target_results)))
            print "2. Execute HMD"
            ...
            for idx in range(0, len(target_results), split_cnt):
                proc = Process(target=extract_error_call_hmd, args=(
            ...
            oracle = connect_db(logger, 'Oracle')
            oracle.delete_error_call_hmd_result(call_date)
            # Insert error call hmd result
...
def main(target_call_date):
    """
    This is a program that error call hnd
    :param          target_call_date:        Target call date
    """
    global ST
    global DT
