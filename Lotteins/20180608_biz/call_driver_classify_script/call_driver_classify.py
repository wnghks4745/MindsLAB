...
class Oracle(object):
    ...
    def select_call_driver_classify_list(self, call_date, project_code):
        query = """
            SELECT
                AA.CALL_ID,
                AA.RUSER_ID,
            ...
            WHERE 1=1
                ...
                AND CC.STT_RESULT_ID = BB.STT_RESULT_ID
    ...
    def select_category_id(self, name, id):
        query = """
            SELECT
                ID
            FROM
    ...
    def select_call_driver_classify_category_id(self, name):
        query = """
            SELECT
                ID
            FROM
    ...
    def insert_call_driver_classify_hmd_result(self, call_date, project_code, return_list):
        name = 'call_driver_classify_cs' if project_code == 'PC0001' else 'call_driver_classify_tm'
        for hmd_result, temp_dict in return_list:
            try:
                for cls in hmd_result.keys():
                    ...
                    if len(cat_list) != 3:
                        continue
                    id = self.select_call_driver_classify_category_id(name)
                    if not id:
                        continue
                    category_1depth, category_2depth, category_3depth = cat_list
                    category_1depth_id = self.select_category_id(category_1depth, id)
                    if not category_1depth_id:
                        continue
                    category_2depth_id = self.select_category_id(category_2depth, category_1depth_id)
                    if not category_2depth_id:
                        continue
                    category_3depth_id = self.select_category_id(category_3depth, category_2depth_id)
    ...
    def insert_call_driver_classify_hmd_result_by_executemany(self, values_list):
        query = """
        INSERT INTO CS_CALL_DRIVER_CLASSIFY_TB
        (
            PIPELINE_EVENT_ID,
    ...
    def delete_call_driver_classify_hmd_result(self, call_date, project_code):
        try:
            query = """
                DELETE FROM
...
def extract_call_driver_classify_hmd(logger, idx, pro_id, model_params, target_results, return_dict):
    """
    Extract call driver classify hmd
    :param              logger:                     Logger
    :param              idx:                        Index
    ...
    for target_result in target_results:
        try:
            ...
            ruser_id = target_result[1]
            business_dcd = target_result[2]
            pipeline_event_id = target_result[3]
            sentence = target_result[4]
            stt_result_detail_id = target_result[5]
            nlp_result_id = target_result[6]
    ...
    for hmd_result, temp_dict in return_list:
        for cls in hmd_result.keys():
            try:
                ...
                if len(cat_list) != 3:
                    continue
                id = oracle.select_call_driver_classify_category_id(name)
                if not id:
                    continue
                category_1depth, category_2depth, category_3depth = cat_list
                category_1depth_id = oracle.select_category_id(category_1depth, id)
                if not category_1depth_id:
                    continue
                category_2depth_id = oracle.select_category_id(category_2depth, category_1depth_id)
                if not category_2depth_id:
                    continue
                category_3depth_id = oracle.select_category_id(category_3depth, category_2depth_id)
                if not category_3depth_id:
                    continue
                values_tuple = (
                    temp_dict['pipeline_event_id'],
                ...
                if len(value_list) == 10000:
                    ...
                    oracle.insert_call_driver_classify_hmd_result_by_executemany(value_list)
                    value_list = list()
            except Exception:
                print traceback.format_exc()
                logger.error(traceback.format_exc())
...
def processing(target_call_date, pro_code):
    ...
    try:
        ...
        model_params = 'call_driver_classify_cs_hmd' if pro_code == 'CS' else 'call_driver_classify_tm_hmd'
        ...
        target_results = oracle.select_call_driver_classify_list(call_date, project_code)
        oracle.disconnect()
        if target_results:
            ...
            for idx in range(0, len(target_results), split_cnt):
                proc = Process(target=extract_call_driver_classify_hmd, args=(
            ...
            oracle = connect_db(logger, 'Oracle')
            oracle.delete_call_driver_classify_hmd_result(call_date, project_code)
            # Insert call driver classify hmd result
