...
class Oracle(object):
    ...
    def select_negative_keyword_call_list(self, call_date, project_code):
        query = """
            SELECT
                AA.CALL_ID,
                AA.BUSINESS_DCD,
    ...
    def insert_negative_keyword_hmd_result(self, call_date, project_code, return_list):
    ...
    def insert_negative_keyword_hmd_result_by_executemany(self, value_list):
        query = """
        INSERT INTO CS_NEGATIVE_KEYWORD_TB
        (
            PIPELINE_EVENT_ID,
    ...
    def delete_negative_keyword_hmd_result(self, call_date, project_code):
        try:
            query = """
                DELETE FROM
                    CS_NEGATIVE_KEYWORD_TB
                WHERE 1=1
                    AND CALL_DATE = TO_DATE(:1, 'YYYY-MM-DD')
                    AND PROJECT_CODE = :2
            """
...
def extract_negative_keyword_hmd(logger, idx, pro_id, model_params, target_results, return_dict):
    """
    Extract negative keyword hmd
    :param              logger:                     Logger
    :param              idx:                        Index
        ...
        for cls in hmd_result.keys():
            try:
                ...
                if len(value_list) == 10000:
                    ...
                    oracle.insert_negative_keyword_hmd_result_by_executemany(value_list)
                    value_list = list()
            except Exception:
                print traceback.format_exc()
                logger.error(traceback.format_exc())
...
def processing(target_call_date, pro_code):
    ...
    try:
        ...
        model_params = 'negative_keyword_cs_hmd' if pro_code == 'CS' else 'negative_keyword_tm_hmd'
        ...
        target_results = oracle.select_negative_keyword_call_list(call_date, project_code)
        oracle.disconnect()
        if target_results:
            ...
            for idx in range(0, len(target_results), split_cnt):
                proc = Process(target=extract_negative_keyword_hmd, args=(
            ...
            oracle = connect_db(logger, 'Oracle')
            oracle.delete_negative_keyword_hmd_result(call_date, project_code)
            # Insert negative keyword hmd result
