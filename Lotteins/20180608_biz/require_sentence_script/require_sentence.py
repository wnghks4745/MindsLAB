...
class Oracle(object):
    ...
    def select_require_sentence_call_list(self, call_date):
        query = """
            SELECT
                AA.CALL_ID,
                AA.CALL_TYPE_CODE,
    ...
    def insert_require_sentence_hmd_result(self, call_date, return_list):
        cnt = 0
        output_dict = dict()
        print "4. Insert target data"
        self.logger.info("4. Insert target data")
        for hmd_result, temp_dict in return_list:
            call_id = temp_dict['call_id']
            business_dcd = temp_dict['business_dcd']
            pipeline_event_id = temp_dict['pipeline_event_id']
            key = "{0}!@#${1}!@#${2}".format(call_id, business_dcd, pipeline_event_id)
            if key not in output_dict:
                output_dict[key] = [(hmd_result, temp_dict)]
            else:
                output_dict[key].append((hmd_result, temp_dict))
        for key, data in output_dict.items():
            call_id, business_dcd, pipeline_event_id = key.split("!@#$")
            check_category_dict = {
                'open_mention': ['N', '', ''],
                'close_mention': ['N', '', ''],
                'extra_question': ['N', '', ''],
        ...
        self.logger.info("Total upload count : {0}".format(cnt))

    def insert_require_sentence_hmd_result_by_executemany(self, value_list):
        query = """
           INSERT INTO CS_REQUIRE_SENTENCE_TB
           (
               PIPELINE_EVENT_ID,
           ...
           """
        self.cursor.executemany(query, value_list)
        if self.cursor.rowcount > 0:
            self.conn.commit()
        else:
            self.conn.rollback()

    def delete_require_sentence_hmd_result(self, call_date):
        try:
            query = """
                DELETE FROM
                    CS_REQUIRE_SENTENCE_TB
                WHERE 1=1
                    AND CALL_DATE = TO_DATE(:1, 'YYYY-MM-DD')
            """
            bind = (
...
def extract_require_sentence_hmd(logger, idx, pro_id, target_results, return_dict):
    """
    Extract require sentence call hmd
    :param              logger:                     Logger
    :param              idx:                        Index
    ...
    for target_result in target_results:
        try:
            ...
            call_type_code = target_result[1]
            ...
            model_params = 'require_sentence_in_hmd' if call_type_code == 'CT0001' else 'require_sentence_out_hmd'
...
def insert_result_data(logger, oracle, call_date, return_list):
    """
    Insert result data
    :param          logger:             Logger
    :param          oracle:             Oracle
    ...
    for key, data in output_dict.items():
        ...
        if len(value_list) == 10000:
            ...
            oracle.insert_require_sentence_hmd_result_by_executemany(value_list)
            value_list = list()
    if len(value_list) > 0:
        cnt += len(value_list)
...
def processing(target_call_date):
    ...
    try:
        ...
        target_results = oracle.select_require_sentence_call_list(call_date)
        oracle.disconnect()
        if target_results:
            ...
            for idx in range(0, len(target_results), split_cnt):
                proc = Process(target=extract_require_sentence_hmd, args=(
                    logger, idx, pro_id, target_results[idx:split_cnt+idx], return_dict), )
            ...
            oracle = connect_db(logger, 'Oracle')
            oracle.delete_require_sentence_hmd_result(call_date)
            # Insert require sentence hmd result
            oracle.insert_require_sentence_hmd_result(call_date, return_list)
