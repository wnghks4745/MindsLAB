...
class DbCollector(object):
    ...
    def create_common_header(self, **kwargs):
        try:
            ...
            header.call_metadata.updator_id = meta_info[25] if meta_info[2] else 'None'
    ...
    def do_tm_job(self):
        ...
        for item in tm_list:
            try:
                ...
                meta_info, stt_spch_sped_rx, stt_spch_sped_tx = get_tm_meta_info(oracle, rfile_name, rec_id)
                if not meta_info:
                    self.logger.error(
                        "Can't select TM meta information. [REC_ID = {0}, RFILE_NAME = {1}]".format(
                            rec_id, rfile_name))
                    continue
                call_date = meta_info[0].strftime('%Y-%m-%d') if meta_info[0] else ''
                start_time = meta_info[6]
                ruser_id = meta_info[10]
                code_result = select_code_from_tm_call_meta_tb(oracle, ruser_id)
                meta_info += code_result if code_result else ('', '', '', '')
                # Extract OFFICE_HOUR
                ...
                update_tm_meta_trans_status(oracle, rfile_name, 'Y', rec_id)
                oracle.conn.commit()
                self.logger.info(
                ...
                # Update TRANSFER_YN status
                update_tm_meta_trans_status(oracle, rfile_name, 'F', rec_id)
                oracle.conn.commit()
                self.logger.error(traceback.format_exc())
                self.logger.error("[ERROR] CALL_ID : {0}, REC_ID : {1}, RFILE_NAME : {2}".format(
...
def select_code_from_tm_call_meta_tb(oracle, ruser_id):
    """
    Get TM meta information
    :param          oracle:             DB
    :param          ruser_id:           RUSER_ID
    :return:                           CENTERCD, SUBCENTERCD, TEAMCD, USERGROUPCD
    """
    query = """
        SELECT
            USERGROUPCD,
...
def update_tm_meta_trans_status(oracle, rfile_name, status, rec_id):
    """
...
def get_tm_meta_info(oracle, rfile_name, rec_id):
    """
    Get TM meta information
    :param          oracle:             DB
    :param          rfile_name:         RFILE_NAME
    :param          rec_id:             REC_ID
    :return:                            TM meta information, Speech speed rx and tx
    """
    query = """
        SELECT
                ...
                FROM
                    (
                        ...
                        ON 
                            ...
                            AND C.USE_YN = 'Y'
                        WHERE 1=1
                            AND A.USER_ID LIKE CONCAT(SUBSTR(RUSER_ID, 1, 7), '%')
                        ORDER BY
                            A.TM_DCD
                    )
                WHERE ROWNUM = 1
        ...
        WHERE 1=1
            ...
            AND REC_ID = :3
    """
    bind = (rfile_name, rfile_name, rec_id,)
    oracle.cursor.execute(query, bind)
    ...
    if not rows:
        return False, False, False
    return rows[:27], rows[27], rows[28]
