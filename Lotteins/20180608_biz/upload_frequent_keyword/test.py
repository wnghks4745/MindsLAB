...
class Oracle(object):
                ...
                FROM
                    ...
                    (
                        ...
                        WHERE 1=1
                            ...
                            AND PROJECT_CODE = :6
                            AND BUSINESS_DCD IN (SELECT FULL_CODE FROM CM_CD_DETAIL_TB WHERE USE_YN = 'Y')
                    )E
                WHERE 1=1
    ...
    def select_related_keyword_sentence(self, call_date, project_code):
        ...
        FROM
            ...
            (
                ...
                WHERE 1=1
                    AND CALL_DATE = TO_DATE(:7, 'YYYY-MM-DD')
            ) F
        WHERE 1=1
            AND A.NLP_RESULT_ID = B.NLP_RESULT_ID
            AND B.STT_RESULT_DETAIL_ID = C.STT_RESULT_DETAIL_ID
    ...
    def temp_1(self, call_date, project_code):
        query = """
            SELECT 
                A.WORD,
                A.NLP_RESULT_ID,
...
def insert_related_sentence(logger, oracle, call_date, project_code):
    ...
    result_1 = oracle.temp_1(call_date, project_code)
    result_2 = oracle.temp_2(call_date)
    temp_dict_1 = dict()
    temp_dict_2 = dict()
    if result_1 and result_2:
        for item in result_2:
            ...
            speaker_code = item[1]
            business_dcd = item[2]
            nlp_frequent_keyword_id = item[3]
            key = "{0}!@#${1}!@#${2}".format(word, speaker_code, business_dcd)
            if key in temp_dict_1:
                temp_dict_1[key].append(nlp_frequent_keyword_id)
            else:
                temp_dict_1[key] = [nlp_frequent_keyword_id]
        for item in result_1:
            ...
            key = "{0}!@#${1}!@#${2}".format(word, speaker_code, business_dcd)
            if key in temp_dict_1:
                for nlp_frequent_keyword_id in temp_dict_1[key]:
                    overlap_check_key = "{0}!@#${1}!@#${2}!@#${3}".format(
                        nlp_frequent_keyword_id, nlp_result_id, stt_result_detail_id, call_id)
                    temp_dict_2[overlap_check_key] = [1]
        logger.info("6. Insert related keyword sentence")
        logger.info("Related keyword sentence count = {0}".format(len(temp_dict_1)))
        logger.info("Related keyword sentence count = {0}".format(len(temp_dict_2)))
        value_list = list()
        cnt = 0
