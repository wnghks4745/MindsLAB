...
class Oracle(object):
    ...
    def select_related_keyword_sentence(self, call_date, project_code):
        ...
        FROM
            ...
            STT_RESULT_TB D,
            CM_CALL_META_TB E,
            TA_NLP_FREQUENT_KEYWORD_RU_TB F
        WHERE 1=1
            AND A.NLP_RESULT_ID = B.NLP_RESULT_ID
            AND B.STT_RESULT_DETAIL_ID = C.STT_RESULT_DETAIL_ID
            AND C.STT_RESULT_ID = D.STT_RESULT_ID
            ...
            AND C.SPEAKER_CODE = 'ST0002'
            AND F.RUSER_ID = E.RUSER_ID
            AND F.PROJECT_CODE = :4
        """
        bind = (
            call_date,
            call_date,
        ...
        query = """
            ...
            FROM
                (
                    SELECT
                        CC.WORD,
                        AA.NLP_FREQUENT_KEYWORD_RU_ID
                    FROM
                        TA_NLP_RESULT_KEYWORD_TB CC,
                        TA_NLP_FREQUENT_KEYWORD_RU_TB AA,
                        TA_NLP_FREQUENT_KYWD_DETAIL_RU_TB BB,
                    ...
                    WHERE 1=1
                        ...
                        AND AA.NLP_FREQUENT_KEYWORD_RU_ID = BB.NLP_FREQUENT_KEYWORD_RU_ID
                        AND AA.CALL_DATE = TO_DATE(:1, 'YYYY-MM-DD')
                        AND FF.PROJECT_CODE = :2
                        AND AA.PROJECT_CODE = :3
                        AND BB.PROJECT_CODE = :4
                        AND FF.RUSER_ID = AA.RUSER_ID
                        AND AA.WORD != CC.WORD
                )
            GROUP BY
                NLP_FREQUENT_KEYWORD_RU_ID,
    ...
    def insert_frequent_keyword_ru_tb(self, **kwargs):
        query = """
        INSERT INTO TA_NLP_FREQUENT_KEYWORD_RU_TB
        (
            CALL_DATE,
            RUSER_ID,
            PROJECT_CODE,
    ...
    def insert_frequent_keyword_detail_tb(self, values_list):
        query = """
        INSERT INTO TA_NLP_FREQUENT_KYWD_DETAIL_RU_TB
        (
            NLP_FREQUENT_KEYWORD_RU_ID,
            NLP_RESULT_ID,
    ...
    def insert_related_keyword_tb(self, values_list):
        query = """
        INSERT INTO TA_RELATED_KEYWORD_RU_TB
        (
            NLP_FREQUENT_KEYWORD_RU_ID,
            CALL_DATE,
            RELATED_WORD,
    ...
    def delete_frequent_detail_data(self, call_date, project_code):
        ...
        try:
            query = """
                DELETE FROM
                    TA_NLP_FREQUENT_KYWD_DETAIL_RU_TB
                WHERE 1=1
                    AND CALL_DATE = TO_DATE(:1, 'YYYY-MM-DD')
                    AND PROJECT_CODE = :2
            """
...
def based_on_the_word(logger, oracle, call_date, project_code):
    ...
    if result:
        ...
        for item in result:
            ...
            word_cnt = int(item[1])
            ruser_id = item[2]
            if ruser_id not in frequent_dict:
                frequent_dict[ruser_id] = {word : word_cnt}
            else:
                if word in frequent_dict[ruser_id]:
                    frequent_dict[ruser_id][word] += word_cnt
                else:
                    frequent_dict[ruser_id].update({word:word_cnt})
        logger.info("2. Insert keyword frequency by word")
        for ruser_id, word_dict in frequent_dict.items():
            ...
            for word, word_cnt in sorted_frequent_dict:
                oracle.insert_frequent_keyword_ru_tb(
                    call_date=call_date,
...
def based_on_the_document(logger, oracle, call_date, project_code):
    ...
    if result:
        ...
        for item in result:
            ...
            ruser_id = item[2]
            if ruser_id not in frequent_dict:
                frequent_dict[ruser_id] = {word : document_cnt}
            else:
                if word in frequent_dict[ruser_id]:
                    frequent_dict[ruser_id][word] += document_cnt
                else:
                    frequent_dict[ruser_id].update({word:document_cnt})
...
def insert_related_sentence(logger, oracle, call_date, project_code):
    ...
    if result:
        ...
        for item in group_by_dict.keys():
            nlp_frequent_keyword_ru_id = item[0]
            nlp_result_id = item[1]
            stt_result_detail_id = item[2]
            call_id = item[3]
            values_tuple = (
                nlp_frequent_keyword_ru_id, nlp_result_id, stt_result_detail_id,
                call_date, project_code, call_id, CREATOR_ID, CREATOR_ID)
...
def insert_related_keyword(logger, oracle, call_date, project_code):
    ...
    if result:
        for item in result:
            ...
            word = item[1]
            word_cnt = item[2]
            if nlp_frequent_keyword_ru_id not in frequent_dict:
                frequent_dict[nlp_frequent_keyword_ru_id] = {word: word_cnt}
            else:
                if word in frequent_dict[nlp_frequent_keyword_ru_id]:
                    frequent_dict[nlp_frequent_keyword_ru_id][word] += word_cnt
                else:
                    frequent_dict[nlp_frequent_keyword_ru_id].update({word: word_cnt})
        logger.info("8. Insert related keyword")
        cnt = 0
        value_list = list()
        for nlp_frequent_keyword_ru_id, word_dict in frequent_dict.items():
            sorted_frequent_dict = sorted(word_dict.iteritems(), key=itemgetter(1), reverse=True)
...
def processing(pro_code, target_call_date):
    ...
    logger_args = {
        ...
        'log_file_name': "RU_" + pro_code + "_" + CONFIG['log_name'],
