...
"""program"""
__author__ = "MINDsLAB"
__date__ = "creation: 2018-01-09, modification: 2018-05-30"
...
#            temp_tb_tm_qa_ta_dtc_rst_dict[key] = tb_tm_qa_ta_dtc_rst_dict
        if key not in TB_TM_QA_TA_DTC_RST_DICT:
            TB_TM_QA_TA_DTC_RST_DICT[key] = tb_tm_qa_ta_dtc_rst_dict
...
def modify_nlp_output_line_number(logger, ctrdt):
    """
    ...
    if not os.path.exists(NLP_DIR_PATH):
    ...
    for dir_path, sub_dirs, files in w_ob:
        for file_name in files:
            ...
            modified_line_number_file = open(os.path.join(NLP_DIR_PATH, file_name), 'w')
            merge_temp_num = 0
            merge_temp_sent = ""
            merge_temp_nlp_sent = ""
...
def make_ne_cnt_file():
    ...
    ne_output_file = open("{0}/{1}.ne.cnt".format(TA_TEMP_DIR_PATH, os.path.basename(TA_TEMP_DIR_PATH)), 'w')
...
def make_morph_cnt_file(logger):
    ...
    morph_output_file = open("{0}/{1}.morph.cnt".format(TA_TEMP_DIR_PATH, os.path.basename(TA_TEMP_DIR_PATH)), 'w')
...
def make_statistics_file(logger):
    ...
    :param      logger:     Logger
    """
    logger.info("3-2. Make statistics file")
    logger.info("3-2-1. Make morph.cnt file")
    make_morph_cnt_file(logger)
    logger.info("3-2-2. Make ne.cnt file")
    make_ne_cnt_file()
...
def execute_new_lang(logger):
    ...
    """
    global DELETE_FILE_LIST
    logger.info("3-1. Execute new_lang.exe")
    start = 0
    end = 0
    cmd_list = list()
    ...
    for dir_name in output_dir_list:
        ...
        if not os.path.exists(output_dir_path):
            os.makedirs(output_dir_path)
    temp_new_lang_dir_path = '{0}/{1}'.format(QA_TA_CONFIG['ta_bin_path'], TA_TEMP_DIR_PATH)
    ...
    for cnt in range(thread):
        ...
        list_file_path = "{0}/{1}_{2}.list".format(temp_new_lang_dir_path, os.path.basename(TA_TEMP_DIR_PATH), cnt)
        list_file = open(list_file_path, 'w')
        for idx in range(start, end):
...
def copy_stt_and_nlp_output(logger, rec_info_dict):
    """
    Copy STT output
    :param      logger:                 Logger
...
#            target_file = '{0}/modified_nlp_line_number/{1}.hmd.txt'.format(stt_output_dir_path, rfile_name)
        txt_file = '{0}/txt/{1}_trx.txt'.format(stt_output_dir_path, rfile_name)
...
#            logger.error('{0} is not exists'.format(target_file))
        if not os.path.exists(txt_file):
            logger.error('{0} is not exists'.format(txt_file))
...
#        shutil.copy(target_file, NLP_DIR_PATH)
        logger.info('\tcopy {0} -> {1}'.format(txt_file, TXT_DIR_PATH))
        shutil.copy(txt_file, TXT_DIR_PATH)
...
#    NLP_DIR_PATH = '{0}/modify_nlp_line_number'.format(TA_TEMP_DIR_PATH)
    ...
    TXT_DIR_PATH = '{0}/txt'.format(TA_TEMP_DIR_PATH)
#    os.makedirs(NLP_DIR_PATH)
...
def processing(job):
    ...
    try:
        ...
        copy_stt_and_nlp_output(logger, rec_info_dict)
        # 3-1. Execute TA
        execute_new_lang(logger)
        # 3-2. Make statistics file
        make_statistics_file(logger)
        # 3-3. Modify nlp output
        modify_nlp_output_line_number(logger, ctrdt)
        # 4. Make matrix file
