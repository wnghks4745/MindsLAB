...
if __name__ == '__main__':
	...
	if len(sys.argv) != 2:
		filelists = os.popen('ls *.idx.srt.idxprn')
	else:
		filelists = os.popen('ls '+sys.argv[1]+'/*.idx.srt.idxprn')
	...
	for filelist in filelists:
		...
		if media == 'News':
			...
			vmedia = filelist[:-1].split('.idx.srt')[0].split('/')
			media = vmedia[len(vmedia)-1]


		#base_name = os.path.basename(filelist[:-1])
		output_file_keyword = open(filelist[:-1].split('.idx.srt')[0]+'_idxprn_keyword.csv', 'w')
		output_file_sentiment = open(filelist[:-1].split('.idx.srt')[0]+'_idxprn_sentiment.csv', 'w')
		...
		for line in lines:
			...
			for key in tmp.keys():
				#cur.execute("INSERT INTO new_keyword_tbl (channel, doc_id, sent_id, keyword, type, sent_cnt, doc_date, mon) VALUES ("+media+", "+line[4]+", "+key+", '"+line[1]+"', "+line[2]+", "+str(tmp[key][0])+", "+line[0]+", "+line[0][0:6]+");")
				...
				#output_file_keyword.write("ivoc\t"+line[4]+"\t"+key+"\t"+line[1]+"\t"+line[2]+"\t"+str(tmp[key][0])+"\t"+line[0]+"\t"+media+"\t"+line[0][0:6]+"\n")
				#output_file_sentiment.write("ivoc\t"+line[4]+"\t"+key+"\t"+tmp[key][1]+"\t"+tmp[key][2]+"\t"+line[0]+"\t"+media+"\t"+line[0][0:6]+"\n")
				output_file_keyword.write(line[4]+"\t"+key+"\t"+line[1]+"\t"+line[2]+"\t"+str(tmp[key][0])+"\t"+line[0]+"\t"+media+"\t"+line[0][0:6]+"\n")
