...
from cfg.config import ORACLE_DB_CONFIG
from cfg.config import MASKING_CONFIG
from lib.iLogger2 import set_logger
...
class Oracle(object):
    ...
    def select_marketing_rec_meta_tb(self, task_date, file_name):
        query = """
            SELECT
                *
            FROM
            ...
            WHERE 1=1
                AND TASK_DATE = TO_DATE(:1, 'YYYY-MM-DD HH24:MI:SS')
                AND FILE_NAME = :2
        """
        bind = (
            task_date,
    ...
    def insert_tb_tm_stt_rst(self, inset_set_dict):
        try:
            sql = """
                ...
                VALUES (
                    TO_DATE(:1, 'YYYY-MM-DD HH24:MI:SS'), :2, :3, :4, :5, :6, SYSDATE, SYSDATE, 'MA_STT', 'MA_STT'
                )
            """
            values_list = list()
            for insert_dict in inset_set_dict.values():
                task_date = str(insert_dict['TASK_DATE'])
                ...
                values_tuple = (
                    task_date, file_name, stt_sntc_lin_no, stt_sntc_cont, stt_sntc_sttm, stt_sntc_endtm,
                )
    ...
    def update_stt_spch_sped(self, task_date, file_name, stt_spch_sped_rx, stt_spch_sped_tx):
        try:
            query = """
                UPDATE
                    MARKETING_REC_META_TB
    ...
    def update_stt_prgst_cd(self, stt_prgst_cd, task_date, file_name):
        try:
            query = """
                UPDATE
                    MARKETING_REC_META_TB
                SET
                    ...
                    UPDATOR_ID = 'MA_STT'
                WHERE 1=1
                    AND TASK_DATE = TO_DATE(:2, 'YYYY-MM-DD HH24:MI:SS')
                    AND FILE_NAME = :3
            """
            bind = (
                stt_prgst_cd,
    ...
    def delete_marketing_rec_stt_rst(self, task_date, file_name):
        try:
            query = """
                DELETE FROM
                    MARKETING_REC_STT_RST
    ...
    def delete_data_to_tb_tm_stt_slce_dtc_rst(self, task_date, file_name):
        try:
            query = """
                DELETE FROM
...
def error_process(logger, oracle, task_date, file_name, stt_prgst_cd, biz_cd):
    """
    Error process
    :param      logger:                 Logger
    :param      oracle:                 Oracle db
    :param      task_date:              TASK_DATE(녹취 ID)
    :param      file_name:              FILE_NAME(녹취파일명)
    :param      stt_prgst_cd:           STT_PRGST_CD(STT 상태코드)
    :param      biz_cd:                 BIZ_CD(업체구분코드)
    """
    logger.error("Error process")
    logger.error("TASK_DATE = {0}, FILE_NAME = {1}, change STT_PRGST_CD = {2}".format(
        task_date, file_name, stt_prgst_cd))
    oracle.update_stt_prgst_cd(stt_prgst_cd, task_date, file_name)
    rec_path = '{0}/TM/{1}'.format(STT_CONFIG['rec_dir_path'], biz_cd)
    if not stt_prgst_cd == '00':
        target_rec_path_list = glob.glob('{0}/{1}.*'.format(rec_path, file_name))
        target_rec_path_list += glob.glob('{0}/{1}_*'.format(rec_path, file_name))
...
def move_output(logger):
    ...
    for info_dict in RCDG_INFO_DICT.values():
        task_date = str(info_dict['TASK_DATE'])[:10]
        file_name = info_dict['FILE_NAME']
        chn_tp = info_dict['CHN_TP']
        output_dir_path = '{0}/{1}/{2}/{3}/{4}-{5}'.format(
            STT_CONFIG['stt_output_path'], task_date[:4], task_date[5:7], task_date[8:10], task_date, file_name)
        output_dict = {
            'mlf': {'ext': 'mlf', 'merge': 'N'},
        ...
        for target, target_option in output_dict.items():
            ...
            if chn_tp == 'S':
                if target_option['merge'] == 'Y':
                    path_list.append('{0}/{1}/{2}_trx.{3}'.format(STT_TEMP_DIR_PATH, target, file_name, ext))
                else:
                    path_list.append('{0}/{1}/{2}_rx.{3}'.format(STT_TEMP_DIR_PATH, target, file_name, ext))
                    path_list.append('{0}/{1}/{2}_tx.{3}'.format(STT_TEMP_DIR_PATH, target, file_name, ext))
            else:
                path_list.append('{0}/{1}/{2}.{3}'.format(STT_TEMP_DIR_PATH, target, file_name, ext))
            for path in path_list:
                file_names = os.path.basename(path)
                if os.path.exists('{0}/{1}'.format(output_target_path, file_names)):
                    del_garbage(logger, '{0}/{1}'.format(output_target_path, file_names))
...
def extract_silence(start_time_idx, end_time_idx, delimiter, input_line_list, silence_seconds):
    """
    ...
    for idx in range(0, len(input_line_list)):
        ...
        if idx + 1 == len(input_line_list):
            ...
            compared_duration = speaker_last_end_time_dict[compared_speaker] - start_time_seconds
...
def update_marketing_rec_stt_rst(logger, oracle):
    """
    UPDATE MARKETING_REC_STT_RST
    :param      logger:             Logger
    :param      oracle:             Oracle db
    """
    global RCDG_INFO_DICT
    logger.info("11-2. DB update MARKETING_REC_STT_RST")
    insert_set_dict = dict()
    for key, info_dict in RCDG_INFO_DICT.items():
        task_date = info_dict['TASK_DATE']
        file_name = info_dict['FILE_NAME']
        chn_tp = info_dict['CHN_TP']
        biz_cd = info_dict['BIZ_CD']
        if chn_tp == 'M':
            detail_file_path = '{0}/masking/{1}.detail'.format(STT_TEMP_DIR_PATH, file_name)
        else:
            detail_file_path = '{0}/masking/{1}_trx.detail'.format(STT_TEMP_DIR_PATH, file_name)
        if not os.path.exists(detail_file_path):
            logger.error("Can't find detail file -> {0}".format(detail_file_path))
            error_process(logger, oracle, task_date, file_name, '02', biz_cd)
        ...
        detail_file.close()
        # 무음 처리 구간 조회
        silence_result = extract_silence(1, 2, "\t", detail_file_list, STT_CONFIG['silence_seconds'])
        # DB insert 전 delete
        oracle.delete_marketing_rec_stt_rst(task_date, file_name)
        line_num = 0
        rx_sntc_len = 0
        tx_sntc_len = 0
        rx_during_time = 0
        ...
        for line in detail_file_list:
            ...
            insert_dict['TASK_DATE'] = task_date
            insert_dict['FILE_NAME'] = file_name
            ...
            keyword = '{0}_{1}_{2}'.format(task_date, file_name, line_num)
...
#        oracle.update_stt_spch_sped(task_date, file_name, stt_spch_sped_rx, stt_spch_sped_tx)
    oracle.insert_tb_tm_stt_rst(insert_set_dict)


def update_marketing_rec_meta_tb(logger, oracle):
    """
    UPDATE MARKETING_REC_META_TB
    :param      logger:             Logger
    :param      oracle:             Oracle db
    """
    global RCDG_INFO_DICT
    logger.info("11-1. DB update MARKETING_REC_META_TB")
    for key, info_dict in RCDG_INFO_DICT.items():
        task_date = info_dict['TASK_DATE']
        file_name = info_dict['FILE_NAME']
        oracle.update_stt_prgst_cd('03', task_date, file_name)
...
def make_output(logger, oracle, target_dir_path):
    ...
    for key, info_dict in RCDG_INFO_DICT.items():
        ...
        # Mono
        if chn_tp == 'M':
            file_path = '{0}/{1}.stt'.format(target_dir_path, file_name)
            if os.path.exists(file_path):
                stt_file = open(file_path, 'r')
                txt_output_file = open('{0}/{1}.txt'.format(txt_dir_path, file_name), 'w')
                detail_output_file = open('{0}/{1}.detail'.format(detail_dir_path, file_name), 'w')
                ...
                logger.error("{0} don't have stt file.".format(file_name))
                error_process(logger, oracle, task_date, file_name, '02', biz_cd)
                del RCDG_INFO_DICT[key]
                continue
        # Stereo
        else:
            rx_file_path = '{0}/{1}_rx.stt'.format(target_dir_path, file_name)
            tx_file_path = '{0}/{1}_tx.stt'.format(target_dir_path, file_name)
            ...
            # Make mlf info
            rx_mlf_file_path = "{0}/mlf/{1}_rx.mlf".format(STT_TEMP_DIR_PATH, file_name)
            tx_mlf_file_path = "{0}/mlf/{1}_tx.mlf".format(STT_TEMP_DIR_PATH, file_name)
            if not os.path.exists(rx_mlf_file_path) or not os.path.exists(tx_mlf_file_path):
                logger.error("{0} don't have mlf file.".format(file_name))
                error_process(logger, oracle, task_date, file_name, '02', biz_cd)
                del RCDG_INFO_DICT[key]
                continue
            ...
            pre_ed_time = ""
            pre_sent = ""
            txt_output_file = open('{0}/{1}_trx.txt'.format(txt_dir_path, file_name), 'w')
            detail_output_file = open('{0}/{1}_trx.detail'.format(detail_dir_path, file_name), 'w')
...
def execute_unseg_and_do_space(logger, oracle):
    ...
    if len(mlf_list) != mlf_cnt:
        ...
        for key, info_dict in RCDG_INFO_DICT.items():
            ...
            file_name = info_dict['FILE_NAME']
            biz_cd = info_dict['BIZ_CD']
            error_process(logger, oracle, task_date, file_name, '00', biz_cd)
        oracle.disconnect()
...
def make_pcm_file(logger, oracle):
    ...
    for key, info_dict in RCDG_INFO_DICT.items():
        ...
        try:
            ...
            chn_tp = info_dict['CHN_TP']
            file_path = '{0}/{1}.{2}'.format(STT_TEMP_DIR_PATH, file_name, rec_ext)
            rx_file_path = '{0}/{1}_rx.{2}'.format(STT_TEMP_DIR_PATH, file_name, rec_ext)
            tx_file_path = '{0}/{1}_tx.{2}'.format(STT_TEMP_DIR_PATH, file_name, rec_ext)
            # separation rec file
            if chn_tp == 'S' and file_sprt == 'N':
                logger.debug("\tSeparation rec file")
                if rec_ext == 'pcm':
                    wav_file_path = '{0}/{1}.wav'.format(STT_TEMP_DIR_PATH, file_name)
                    rx_file_path = '{0}/{1}_rx.wav'.format(STT_TEMP_DIR_PATH, file_name)
                    tx_file_path = '{0}/{1}_tx.wav'.format(STT_TEMP_DIR_PATH, file_name)
    ...
    for key, info_dict in RCDG_INFO_DICT.items():
        ...
        try:
            ...
            if chn_tp == 'M' or (chn_tp == 'S' and file_sprt == 'N'):
                file_path = '{0}/{1}.{2}'.format(target_dir, file_name, rec_ext)
                if not os.path.exists(file_path):
                    logger.error("{0} file is not exist -> {1}".format(rec_ext, file_path))
                    error_process(logger, oracle, task_date, file_name, '90', biz_cd)
                    del RCDG_INFO_DICT[key]
                    continue
            ...
            # Stereo
                ...
                rx_file_path = '{0}/{1}_rx.{2}'.format(target_dir, file_name, rec_ext)
                tx_file_path = '{0}/{1}_tx.{2}'.format(target_dir, file_name, rec_ext)
                if not os.path.exists(rx_file_path) or not os.path.exists(tx_file_path):
                ...
                logger.error("CHN_TP ERROR {0} : {1}".format(task_date, chn_tp))
                error_process(logger, oracle, task_date, file_name, '90', biz_cd)
                del RCDG_INFO_DICT[key]
                continue
        except Exception:
...
def update_status_and_select_rec_file(logger, oracle, job_list):
    ...
    for job in job_list:
        ...
        biz_cd = 'Marketing'
        try:
            oracle.update_stt_prgst_cd('01', task_date, file_name)
            result_dict = oracle.select_marketing_rec_meta_tb(task_date, file_name)
            if not result_dict:
                raise Exception("No data TM rcdg information to TB_QA_STT_TM_RCDG_INFO,"
                                " TASK_DATE = {0}, FILE_NAME = {1}".format(task_date, file_name))
            key = '{0}&{1}'.format(task_date, file_name)
            result_dict['BIZ_CD'] = biz_cd
            file_name, ext = os.path.splitext(result_dict['FILE_NAME'])
            result_dict['FILE_NAME'] = file_name
            result_dict['CHN_TP'] = 'M'
            result_dict['REC_EXT'] = 'wav'
            result_dict['PROJECT_CD'] = 'TM'
            result_dict['FILE_SPRT'] = 'N'
...
def setup_data():
    ...
    cnt = 0
    while True:
        STT_TEMP_DIR_PATH = "{0}/marketing_stt_temp_directory_{1}".format(STT_CONFIG['stt_path'], cnt)
...
def processing(job_list):
    """
    STT processing
    :param      job_list:        Job(TASK_DATE, FILE_NAME, STT_PRGST_CD)
    """
    # 0. Setup data
    ...
    try:
        ...
        # 11. DB upload
        try:
            update_marketing_rec_meta_tb(logger, oracle)
            update_marketing_rec_stt_rst(logger, oracle)
        except Exception:
        ...
        for info_dict in RCDG_INFO_DICT.values():
            ...
            if chn_tp == 'M' or (chn_tp == 'S' and file_sprt == 'N'):
                target_file_list = [
                    '{0}/{1}/{2}/{3}.{4}'.format(STT_CONFIG['rec_dir_path'], project_cd, biz_cd, file_name, rec_ext)
                ]
            else:
                target_file_list = [
                    '{0}/{1}/{2}/{3}_rx.{4}'.format(STT_CONFIG['rec_dir_path'], project_cd, biz_cd, file_name, rec_ext),
                    '{0}/{1}/{2}/{3}_tx.{4}'.format(STT_CONFIG['rec_dir_path'], project_cd, biz_cd, file_name, rec_ext)
                ]
            for target_file in target_file_list:
