...
from cfg.config import CONFIG, ORACLE_DB_CONFIG, MSSQL_DB_CONFIG, ORACLE_DB_CONFIG_BIZ
...
class Oracle(object):
    ...
    def check_tb_tm_stt_rcdg_info(self, rec_id, rfile_name):
        try:
            query = """
                SELECT
                    *
    ...
    def insert_data_to_rcdg_info(self, **kwargs):
        try:
            query = """
                INSERT INTO TB_TM_STT_RCDG_INFO
                (
                    REC_ID,
                    RFILE_NAME,
                    JOIN_ARC_DCD,
                ...
                VALUES (
                    :1, :2, :3, :4, SYSDATE, :5, :6, :7, :8, :9, :10, :11, TO_DATE(:12, 'YYYY/MM/DD HH24:MI:SS'), 
                    TO_DATE(:13, 'YYYY/MM/DD HH24:MI:SS'), :14, :15, :16, :17, :18, M2U_MASKING_NAME(:19), 
                    M2U_CRYPTO_SHA256_ENCRYPT(:20), :21, :22, :23, :24, :25, :26, :27, :28, SYSDATE, :29, :30, 
                    SYSDATE
                )
            """
            bind = (
                ...
                kwargs.get('join_arc_dcd'),
                kwargs.get('cntr_proc_dcd'),
                kwargs.get('stt_cmdtm'),
                kwargs.get('stt_server_id'),
                kwargs.get('nqa_stta_prgst_cd'),
                kwargs.get('nqa_sel_ta_prgst_cd'),
    ...
    def update_data_to_rcdg_info(self, **kwargs):
        try:
            query = """
                UPDATE
                    TB_TM_STT_RCDG_INFO
                SET
                    ...
                    STT_SERVER_ID = :3,
                    NQA_STTA_PRGST_CD = :4,
                    NQA_SEL_TA_PRGST_CD = :5,
                    PROJECT_CD = :6,
                    CTI_CALL_ID = :7,
                    CALL_TYPE = :8,
                    CALL_START_TIME = TO_DATE(:9, 'YYYY/MM/DD HH24:MI:SS'),
                    CALL_END_TIME = TO_DATE(:10, 'YYYY/MM/DD HH24:MI:SS'),
                    CALL_DURATION = :11,
                    RUSER_ID = :12,
                    RUSER_NAME = :13,
                    RUSER_NUMBER = :14,
                    CU_ID = :15,
                    CU_NAME = M2U_MASKING_NAME(:16),
                    CU_NAME_HASH = M2U_CRYPTO_SHA256_ENCRYPT(:17),
                    CU_NUMBER = :18,
                    IN_CALL_NUMBER = :19,
                    BIZ_CD = :20,
                    CHN_TP = :21,
                    ...
                    LST_CHG_DTM = SYSDATE
                WHERE 1=1
                    AND TB_TM_STT_RCDG_INFO.REC_ID = :26
                    AND TB_TM_STT_RCDG_INFO.RFILE_NAME = :27
            """
            bind = (
                kwargs.get('join_arc_dcd'),
                kwargs.get('stt_cmdtm'),
...
class Oracle2(object):
    def __init__(self, logger):
        self.logger = logger
        self.dsn_tns = cx_Oracle.makedsn(
            ORACLE_DB_CONFIG_BIZ['host'],
            ORACLE_DB_CONFIG_BIZ['port'],
            sid=ORACLE_DB_CONFIG_BIZ['sid']
        )
        self.conn = cx_Oracle.connect(
            ORACLE_DB_CONFIG_BIZ['user'],
            ORACLE_DB_CONFIG_BIZ['passwd'],
            self.dsn_tns
        )
        self.cursor = self.conn.cursor()
...
def upload_data_to_db(logger, oracle, oracle_biz, checked_json_data_dict):
    """
    Upload data to database
    :param          logger:                         Logger
    :param          oracle:                         Oracle
    :param          oracle_biz:                     Oracle for biz
        ...
        try:
            ...
            if project_code == 'PC0001':
                ...
                if user_id == 'None':
                    ...
                    result = oracle_biz.select_cs_business_dcd(user_id, user_name)
                    if result:
            ...
            # TM BUSINESS_DCD
                ...
                if user_id == 'None':
                    ...
                    result = oracle_biz.select_tm_business_dcd(user_id, user_name)
                    if result:
                        business_dcd = result[0]
                    else:
                        business_dcd = ''
            if business_dcd:
                meta_business_dcd = oracle_biz.select_business_dcd_from_cm_cd_detail_tb(business_dcd)
                if meta_business_dcd:
                    business_dcd = meta_business_dcd[0]
            #oracle.delete_cm_call_meta_tb(json_data.get('r_file_name'))
            oracle_biz.insert_data_to_cm_call_meta_tb(
            ...
            if oracle.check_tb_tm_stt_rcdg_info(json_data.get('rec_id'), json_data.get('r_file_name')):
                oracle.update_data_to_rcdg_info(
                    ...
                    stt_cmdtm='',
                    stt_server_id=str(socket.gethostname()),
                    nqa_stta_prgst_cd='00',
                    nqa_sel_ta_prgst_cd='09',
...
def connect_db(logger, db, unique_number=1):
    """
    Connect database
    :param          logger:             Logger
    :param          db:                 Database
    :param          unique_number       Unique Number
    :return                             SQL Object
    """
    # Connect DB
...
def processing():
    ...
    try:
        ...
        oracle_biz = connect_db(logger, 'Oracle', 2)
        if not oracle:
    ...
    try:
        ...
        if len(sorted_json_list) > 0:
            ...
            upload_data_to_db(logger, oracle, oracle_biz, checked_json_data_dict)
            logger.info(
