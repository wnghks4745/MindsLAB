...
"""program"""
__author__ = "MINDsLAB"
__date__ = "creation: 2018-02-09, modification: 2018-05-01"
...
STT_TEMP_DIR_PATH = ""
STT_TEMP_DIR_NAME = ""
TB_CS_STT_SLCD_DTC_RST_DICT = dict()
...
class Oracle(object):
    ...
    def select_tb_cs_stt_rcdg_info(self, rec_id, rfile_name):
        query = """
            SELECT
                *
            FROM
    ...
    def insert_tb_cs_stt_rst(self, inset_set_dict):
        try:
            sql = """
                INSERT INTO
                    TB_CS_STT_RST
                ...
                VALUES (
                    ...
                    :6, :7, :8, :9, :10,
                    :11, :12,
                    'CS_STT', 'CS_STT', SYSDATE, 'CS_STT', 'CS_STT',
                    SYSDATE
                )
            """
            values_list = list()
    ...
    def insert_data_to_tb_cs_stt_slce_dtc_rst(self, **kwargs):
        try:
            query = """
                INSERT INTO TB_CS_STT_SLCE_DTC_RST
                (
                    REC_ID,
                    RFILE_NAME,
                    STT_SNTC_ST_NO,
                ...
                VALUES (
                    :1, :2, :3, :4, :5, 'CS_STT', 'CS_STT',
                    SYSDATE, 'CS_STT', 'CS_STT', SYSDATE
                )
            """
            bind = (
                kwargs.get('rec_id'),
    ...
    def delete_tb_cs_stt_rst(self, rec_id, rfile_name):
        try:
            query = """
                DELETE FROM
                    TB_CS_STT_RST
    ...
    def delete_data_to_tb_cs_stt_slce_dtc_rst(self, rec_id, rfile_name):
        try:
            query = """
                DELETE FROM
                    TB_CS_STT_SLCE_DTC_RST
                WHERE 1=1
                    AND REC_ID = :1
                    AND RFILE_NAME = :2
            """
...
def error_process(logger, oracle, rec_id, rfile_name, cs_stta_prgst_cd, biz_cd):
    ...
    if not cs_stta_prgst_cd == '00':
        rec_path = '{0}/CS/{1}'.format(STT_CONFIG['rec_dir_path'], biz_cd)
...
def insert_tb_cs_stt_slce_dtc_rst(logger, oracle):
    """
    Insert data to TB_CS_STT_SLCE_DTC_RST
    :param      logger:     Logger
    :param      oracle:     Oracle DB
    """
    logger.info("14-3. DB upload TB_CS_STT_SLCE_DTC_RST")
    logger.debug("Extract silence list")
    for key, silence_result in TB_CS_STT_SLCD_DTC_RST_DICT.items():
        rec_id = key.split("-")[0]
        rfile_name = key.split("-")[1]
        logger.debug("  REC_ID = {0}, RFILE_NAME = {1}".format(rec_id, rfile_name))
        oracle.delete_data_to_tb_cs_stt_slce_dtc_rst(rec_id, rfile_name)
        for silence_key, duration in silence_result.items():
            ...
            oracle.insert_data_to_tb_cs_stt_slce_dtc_rst(
                rec_id=rec_id,
...
def update_tb_cs_stt_rst(logger, oracle):
    """
    UPDATE TB_CS_STT_RST
    :param      logger:             Logger
    :param      oracle:             Oracle db
    """
    global RCDG_INFO_DICT
    global TB_CS_STT_SLCD_DTC_RST_DICT
    logger.info("14-2. DB update TB_CS_STT_RST")
    ...
    for key, info_dict in RCDG_INFO_DICT.items():
        ...
        if len(silence_result) > 0:
            ...
            TB_CS_STT_SLCD_DTC_RST_DICT[slcd_key] = silence_result
        # DB insert 전 delete
        oracle.delete_tb_cs_stt_rst(rec_id, rfile_name)
        line_num = 0
        rx_sntc_len = 0
        tx_sntc_len = 0
        rx_during_time = 0
    ...
    oracle.insert_tb_cs_stt_rst(insert_set_dict)


def update_tb_cs_stt_rcdg_info(logger, oracle):
    """
    UPDATE TB_CS_STT_RCDG_INFO
    :param      logger:             Logger
    :param      oracle:             Oracle db
    """
    global RCDG_INFO_DICT
    logger.info("14-1. DB update TB_CS_STT_RCDG_INFO")
    for key, info_dict in RCDG_INFO_DICT.items():
        rec_id = info_dict['REC_ID']
        rfile_name = info_dict['RFILE_NAME']
        oracle.update_cs_stta_prgst_cd('03', rec_id, rfile_name)
...
def update_status_and_select_rec_file(logger, oracle, job_list):
    ...
    for job in job_list:
        ...
        biz_cd = job['BIZ_CD']
        try:
            oracle.update_cs_stta_prgst_cd('01', rec_id, rfile_name)
            result_dict = oracle.select_tb_cs_stt_rcdg_info(rec_id, rfile_name)
            if not result_dict:
                raise Exception("No data CS rcdg information to TB_CS_STT_RCDG_INFO,"
...
def processing(job_list):
    """
    CS processing
    :param      job_list:        Job(REC_ID, RFILE_NAME, CS_STTA_PRGST_CD)
    """
    # 0. Setup data
    ...
    try:
        ...
        # 14. DB upload
        try:
            update_tb_cs_stt_rcdg_info(logger, oracle)
            update_tb_cs_stt_rst(logger, oracle)
            insert_tb_cs_stt_slce_dtc_rst(logger, oracle)
        except Exception:
    ...
    if len(RCDG_INFO_DICT.keys()) > 0:
        ...
        for key, info_dict in RCDG_INFO_DICT.items():
            rec_id = info_dict['REC_ID']
            rfile_name = info_dict['RFILE_NAME']
            oracle.update_cs_stta_prgst_cd('13', rec_id, rfile_name)
    for info_dict in RCDG_INFO_DICT.values():
        chn_tp = info_dict['CHN_TP']
        biz_cd = info_dict['BIZ_CD']
        file_sprt = info_dict['FILE_SPRT']
