...
class Oracle(object):
    ...
    def delete_summary_data(self, call_date):
        try:
            query = """
                DELETE FROM
        ...
        for project_code in ['PC0001', 'PC0002']:
            ...
            if business_dcd_list:
                for business_dcd_result in business_dcd_list:
                    ...
                    result = oracle.select_call_driver_frequency_by_multi(call_date, project_code, business_dcd)
                    ...
                    if call_id_list:
                        ...
                        for item in call_id_list:
                            ...
                            logger.info("CALL_DATE = {0}, PROJECT_CODE = {1}, BUSINESS_DCD = {2}".format(
                                call_date, project_code, business_dcd))
                            if result:
                                key = "{0}_{1}_{2}".format(result[0], result[1], result[2])
