...
class DbCollector(object):
    def __init__(self):
        ...
        self.logger = logger.get_timed_rotating_logger(
            logger_name='tm_db_collector',
            file_name=os.path.join(os.getenv('MAUM_ROOT'), 'logs/tm_db_collector_process.log'),
            backup_count=5,
            log_level='debug'
        )
        self.conf = Config()
    ...
    def do_tm_job(self):
        ...
        # Select TM target list
        start_time = datetime.now()
        tm_list = get_tm_list(oracle, self.select_cnt)
        end_time = datetime.now()
        print "Get TM list required time = {0}".format(end_time - start_time)
        for item in tm_list:
            call_id = ""
            (rfile_name,) = item
            try:
                ...
                start_time = datetime.now()
                meta_info_temp = get_tm_meta_info(oracle, rfile_name)
                end_time = datetime.now()
                print "Get meta info required time = {0}".format(end_time - start_time)
                if not meta_info_temp:
                    self.logger.error(
                        "Can't select TM meta information. [RFILE_NAME = {0}]".format(rfile_name))
                    continue
                stt_spch_sped_rx = meta_info_temp[28]
                stt_spch_sped_tx = meta_info_temp[29]
                rec_id = meta_info_temp[30]
                meta_info = meta_info_temp[:28]
                # Insert meta information
                start_time = datetime.now()
                call_id = create_call_id(oracle, meta_info)
                end_time = datetime.now()
                print "Create CALL_ID required time = {0}".format(end_time - start_time)
                self.logger.info('[START] CALL_ID : {0}, RFILE_NAME : {1}'.format(call_id, rfile_name))
                # Get stt result
                start_time = datetime.now()
                stt_result = get_tm_stt_result(oracle, rfile_name, rec_id)
                end_time = datetime.now()
                print "Get TM result required time = {0}".format(end_time - start_time)
                for stt_data in stt_result:
                    # Create ticket id
                    start_time = datetime.now()
                    ticket_id = create_ticket_id(oracle, call_id, 'TM_REC', 'TM_REC')
                    end_time = datetime.now()
                    print "Create TICKET_ID required time = {0}".format(end_time - start_time)
                    for pipeline_id, tree in self.tree_map.iteritems():
                        root = tree.get_node(tree.root)
                        if root.data[9] == 'PT0007':
                            # Create pipeline_event_id
                            start_time = datetime.now()
                            pipeline_event_id = create_pipeline_event_id(
                                oracle, pipeline_id, ticket_id, meta_info[24], meta_info[25])
                            end_time = datetime.now()
                            print "Create PIPELINE_EVENT_ID required time = {0}".format(end_time - start_time)
                            ...
                            end_time = datetime.now()
                            print "Insert CS result required time = {0}".format(end_time - start_time)
                            # Get stt detail
                            start_time = datetime.now()
                            stt_detail_list = get_tm_stt_detail(oracle, rfile_name, sc, rec_id)
                            end_time = datetime.now()
                            print "Get CS result detail required time = {0}".format(end_time - start_time)
                            # Insert stt_detail_list
                            start_time = datetime.now()
                            stt_result_detail = insert_stt_detail(oracle, stt_result_id, stt_detail_list)
                            end_time = datetime.now()
                            print "Insert CS result detail required time = {0}".format(end_time - start_time)
                            # Create message
                ...
                update_tm_meta_trans_status(oracle, rfile_name, 'Y')
                oracle.conn.commit()
                self.logger.info("[E N D] CALL_ID : {0}, RFILE_NAME : {1}".format(call_id, rfile_name))
            except Exception:
                oracle.conn.rollback()
                # Update TRANSFER_YN status
                update_tm_meta_trans_status(oracle, rfile_name, 'F')
                oracle.conn.commit()
                self.logger.error(traceback.format_exc())
    ...
    def run(self):
        self.logger.info('[START] TM DB Collector Process started')
...
def insert_stt_detail(oracle, stt_result_id, stt_detail_list):
    ...
    for stt_detail in stt_detail_list:
        ...
        bind = (
            ...
            stt_detail[9],
            stt_detail[10],
            stt_detail[11] if stt_detail[11] != None else 0,
            my_seq,
        )
        print 'bind ----'
        print bind
...
def update_tm_meta_trans_status(oracle, rfile_name, status):
    """
    Update TM meta TRANSFER_YN status
    :param      oracle:             DB
...
def get_tm_stt_detail(oracle, rfile_name, sc, rec_id):
    """
    Get TM CS detail
    :param          oracle:             DB
    :param          rfile_name:         RFILE_NAME
    :param          sc:                 STT_SNTC_SPKR_DCD
    :return:                            TM CS detail result
    """
    query = """
        SELECT
...
def get_tm_stt_result(oracle, rfile_name, rec_id):
    """
    Get TM CS result
    :param          oracle:             DB
    :param          rfile_name:         RFILE_NAME
...
def get_tm_meta_info(oracle, rfile_name):
    """
    Get TM meta information
    :param          oracle:             DB
    :param          rfile_name:         RFILE_NAME
    :return:                            TM meta information
    """
    query = """
        SELECT
                ...
                FROM
                    (
                        SELECT
                            A.TM_DCD
                        FROM
                            GROUP_INFO_TM_USER_TB A
                        INNER JOIN
                            TB_TM_STT_RCDG_INFO B
                        ON
                            B.RFILE_NAME = :1
                        INNER JOIN
                            CM_CD_DETAIL_TB C
                        ON 
                            C.META_CODE = A.TM_DCD
                            AND C.USE_YN = 'Y'
                        WHERE 1=1
                            AND SUBSTR(A.USER_ID, 1, 7) LIKE CONCAT('%', RUSER_ID)
                            AND A.USERNAME = RUSER_NAME
                        ORDER BY
                            A.TM_DCD
                    )
                WHERE ROWNUM = 1
            ) AS BUSINESS_DCD,
            NULL AS RUSER_USERGROUPCD,
            STT_SPCH_SPED_RX,
            STT_SPCH_SPED_TX,
            REC_ID
        FROM 
...
def get_tm_list(oracle, cnt):
    """
    Get target TM list
    :param          oracle:         DB
    :param          cnt:            Select count
    :return:                        Target TM list
    """
    query = """
        SELECT
            RFILE_NAME
                        ...
                        WHERE 1=1
                           ...
                           AND
                                (
                                    ...
                                    AND
                                        TO_TIMESTAMP('20180502100000', 'YYYYMMDDHH24MISS')
                                )
                           AND ROWNUM < 1000000
                        ORDER BY B.LST_CHG_DTM
                    )
