    ...
    try:
        ...
        query = """
            ...
            INNER JOIN
                PL_PROC_TB pc 
            ON
                pr.proc_id = pc.proc_id
            LEFT OUTER JOIN
                PL_PROC_CONFIG_TB conf
            ON
                pr.config_id = conf.config_id
            LEFT OUTER JOIN
                PL_PROC_CONFIG_MODEL_REL_TB cmrel
            ON
                conf.config_id = cmrel.config_id
            AND cmrel.id = (
                                SELECT
                                    id
                                FROM
                                    (
                                        ...
                                        WHERE
                                            sub_rel.config_id = conf.config_id
                                        ORDER BY 
                                            ID ASC
                                    )
                                WHERE ROWNUM = 1
