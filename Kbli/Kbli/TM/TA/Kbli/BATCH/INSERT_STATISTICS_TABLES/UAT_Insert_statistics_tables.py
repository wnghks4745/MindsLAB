...
class Oracle(object):
    ...
    def select_adt_count(self):
        ...
        sql = """
            SELECT
                ...
                , cnt
                , (SELECT B.ORG_PART_C
                      FROM {0}.TCL_ORG_PART@D_KBLUAT_ZREAD B
            ...
            FROM
                ...
                FROM
                    ...
                    , TB_TM_CNTR_RCDE_INFO B
                        LEFT OUTER JOIN {0}.TCL_USER@D_KBLUAT_ZREAD D
                            ON B.CNTC_USID = D.USER_ID
                WHERE 1=1
                --    AND TO_DATE(CAST(A.LST_CHG_DTM AS DATE)) = TO_DATE(SYSDATE - 0)
            ...
            SELECT
                ...
                (
                    SELECT
                        B.ORG_PART_C
                    FROM
                        {0}.TCL_ORG_PART@D_KBLUAT_ZREAD B
                    WHERE
            ...
            FROM
                (
                    ...
                    FROM
                        TB_TM_CNTR_INFO A
                        , TB_TM_CNTR_RCDE_INFO B
                            LEFT OUTER JOIN {0}.TCL_USER@D_KBLUAT_ZREAD D ON B.CNTC_USID = D.USER_ID,
                        TB_TM_STT_RST C
                    WHERE
                        1 = 1
                     --   AND TO_DATE(CAST(A.LST_CHG_DTM AS DATE)) = TO_DATE(SYSDATE - 0)
    ...
    def insert_tb_tm_adt_count(self, logger, result_dict_list):
        """
        금칙어 사용 빈도 통계 저장
        ...
        try:
            ...
            for insert_dict in result_dict_list:
                ...
                values_tuple = (
                    org_fof_c, epl_id, prps_date, cont_no, org_fof_c_nm, prps_user_part_c, prps_user_part_c_nm
                    , prps_cntc_usid, prps_cntc_usid_nm, adt_cd, adt_count, org_fof_c, epl_id, prps_date, cont_no
    ...
    def insert_tb_tm_spch_sped_avg(self, logger, result_dict_list):
        """
        상담원별 발화속도 저장
        :param result_dict_list:
        :return:
        ...
        try:
            ...
            for insert_dict in result_dict_list:
                ...
                values_tuple = (
                    org_fof_c, epl_id, prps_date, cont_no, org_fof_c_nm, prps_user_part_c, prps_user_part_c_nm
                    , prps_cntc_usid, prps_cntc_usid_nm, sntc_spch_sped_avg, org_fof_c, epl_id, prps_date, cont_no
...
def processing():
    ...
    try:
        ...
        if adt_count_dict_list:
            # 금칙어 사용 빈도 통계
            oracle.insert_tb_tm_adt_count(logger, adt_count_dict_list)
        ...
        if spch_sped_avg_dict_list:
            # 상담원별 발화속도
            oracle.insert_tb_tm_spch_sped_avg(logger, spch_sped_avg_dict_list)
