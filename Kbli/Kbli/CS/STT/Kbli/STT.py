...
class Oracle(object):
        ...
        try:
            sql = """
                ...
                WHERE 1=1
                    AND PROJECT_CD = 'CS'
                    AND DOCUMENT_ID = :2
                    AND DOCUMENT_DT = TO_DATE(:3, 'YYYY-MM-DD HH24:MI:SS')
            """
            bind = (prgst_cd, document_id, document_dt,)
            self.cursor.execute(sql, bind)
    ...
    def select_count_to_tb_cs_stt_rst(self, logger, info_dic):
        """
    ...
    def delete_to_tb_cs_stt_rst(self, logger, info_dic):
        """
        delete information on TB_CS_STT_RST table
        :param      logger:             Logger
        :param      info_dic:           Information dictionary
        """
        logger.info('delete  TB_CS_STT_RST information of Recording ID [{0}] File name [{1}]'.format(
            info_dic['REC_ID'], info_dic['DOCUMENT_ID']))
        sql = ''
        try:
            sql = """
            DELETE FROM TB_CS_STT_RST
            WHERE 1=1
                AND REC_ID = :1
                AND RFILE_NAME = :2
            """
    ...
    def insert_stt_result_to_tb_cs_stt_rst(self, logger, output_dict):
        """
        Insert information on TB_CS_STT_RST table
        :param      logger:             Logger
        :param      output_dict:        Output dictionary
        """
        logger.debug('Insert  TB_CS_STT_RST information of Recording ID [{0}] File name [{1}]'.format(
            output_dict['REC_ID'], output_dict['RFILE_NAME']))
        sql = ''
        try:
            sql = """
            INSERT INTO TB_CS_STT_RST
                    (
                        STT_SNTC_LIN_NO
                        , REC_ID
                        , RFILE_NAME
...
def make_stt_result_table_txt(logger, pk, oracle):
    ...
    :param      oracle:                 DB
    """
    logger.debug(RCDG_ID_INFO_DICT)
    info_dic = RCDG_ID_INFO_DICT[pk]
    ...
    if oracle.select_count_to_tb_cs_stt_rst(logger, info_dic):
        oracle.delete_to_tb_cs_stt_rst(logger, info_dic)
    ...
    for line in lines:
        ...
        oracle.insert_stt_result_to_tb_cs_stt_rst(logger, output_dict)
        if stt_sntc_lin_no % 100 == 0:
            if not oracle.commit(logger):
                logger.error('insert_stt_result_to_tb_cs_stt_rst is Fail : document_id -> {0}'.format(document_id))
                raise Exception
        idx += 1
    if not oracle.commit(logger):
        logger.error('insert_stt_result_to_tb_cs_stt_rst is Fail : document_id -> {0}'.format(document_id))
    ...
    try:
        ...
        oracle.update_stt_spch_sped('CS', document_dt, document_id, stt_spch_sped_rx, stt_spch_sped_tx)
    except Exception:
