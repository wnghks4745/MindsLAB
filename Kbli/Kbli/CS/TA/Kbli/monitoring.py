...
class ORACLE(object):
    ...
    def count_call_meta_group(self, call_type, date):
        ...
        sql += """            
            GROUP BY TO_CHAR(DOCUMENT_DT, 'YYYY-MM-DD'), CHAT_TA_PRGST_CD
...
if __name__ == '__main__':
    ...
    for item in oracle.count_call_meta_group(call_type, date):
        if item['CHAT_TA_PRGST_CD'] == '03':
            print '{DT}\t[{CHAT_TA_PRGST_CD}]\tCount: {CNT}\t[ERROR]'.format(**item)
            if detail == 'Y':
                print '=' * 50, '{DT} Error Detail Info '.format(**item), '=' * 50
                print 'index : DOCUMENT_DT | DOCUMENT_ID | REC_ID | CHAT_TA_PRGST_CD'
                idx = 1
                for error_item in oracle.get_call_meta_by_dt_and_cd(call_type, item['DT'], '03'):
                    print '{0} : {DOCUMENT_DT}\t{DOCUMENT_ID}\t{REC_ID}\t{CHAT_TA_PRGST_CD}'.format(idx, **error_item)
                    idx += 1

        else:
            print '{DT}\t[{CHAT_TA_PRGST_CD}]\tCount: {CNT}'.format(**item)
